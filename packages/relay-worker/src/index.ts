...
function runInBackground(ctx, promise) {
...
function notifyPortal(ctx, relayId, event, userId, portalUrl, relaySecret) {
  runInBackground(ctx, fetch(portalUrl + '/relay/notify', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + relaySecret,
...
function notifyPortalLog(ctx, relayId, event, userId, country, errorMessage, portalUrl, relaySecret) {
  if (!userId) return;
  const payload = { relay_id: relayId, event, user_id: userId, country };
  if (errorMessage) payload.error_message = errorMessage;
  runInBackground(ctx, fetch(portalUrl + '/relay/log', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + relaySecret,
...
function classifySocketError(error) {
  ...
  if (message.includes('proxy request failed') || message.includes('cannot connect to the specified address')) {
    return { code: 'unsupported_destination', message };
  }
  if (message === 'TCP ready timeout') return { code: 'tcp_timeout', message };
  if (message.includes('Stream was cancelled')) return { code: 'stream_cancelled', message };
  return { code: 'tcp_error', message };
}
  ...
  const wsToSocket = (async () => {
    try {
      if (initialData && initialData.length > 0) {
        debugLog('[pipe ws→tcp] initialData:', initialData.length, 'bytes');
        await socketWriter.write(initialData);
        stats.wsToTcpBytes += initialData.length;
        resetIdle();
      }
      while (true) {
        ...
        debugLog('[pipe ws→tcp]', bytes.length, 'bytes');
        stats.wsToTcpBytes += bytes.length;
        resetIdle();
        await socketWriter.write(bytes);
      }
      stats.closeReason = 'ws_reader_done';
      debugLog('[pipe ws→tcp] ws reader done');
    } catch (e) {
      const classified = classifySocketError(e);
      stats.closeReason = classified.code;
      if (classified.code === 'stream_cancelled') debugLog('[pipe ws→tcp] cancelled:', classified.message);
      else console.error('[pipe ws→tcp] error:', classified.message);
    } finally {
  ...
  const socketToWs = (async () => {
    try {
      while (true) {
        ...
        const bytes = toUint8Array(value);
        if (bytes.length === 0) continue;
        debugLog('[pipe tcp→ws]', bytes.length, 'bytes');
        stats.tcpToWsBytes += bytes.length;
        resetIdle();
        ws.send(bytes);
      }
      stats.closeReason = 'tcp_reader_done';
      debugLog('[pipe tcp→ws] socket reader done');
    } catch (e) {
      const classified = classifySocketError(e);
      stats.closeReason = classified.code;
      if (classified.code === 'stream_cancelled') debugLog('[pipe tcp→ws] cancelled:', classified.message);
      else console.error('[pipe tcp→ws] error:', classified.message);
    } finally {
...
// ─── Trojan handler ──────────────────────────────────────────────────────────
async function handleTrojanSession(ws, ip, country, ctx) {
  ...
  try {
    ...
    try {
      ...
      socket.closed.then(() => {
        debugLog('[tcp] closed cleanly', 'host:', header.host, 'port:', header.port);
      }).catch((e) => {
        const classified = classifySocketError(e);
        if (classified.code === 'stream_cancelled') debugLog('[tcp] closed cancelled', 'host:', header.host, 'port:', header.port);
        else console.error('[tcp] closed with error:', classified.message, 'host:', header.host, 'port:', header.port);
      });
      ...
      try {
        ...
        const classified = classifySocketError(e);
        if (classified.code === 'tcp_timeout') {
          console.error('[tcp] timeout', 'host:', header.host, 'port:', header.port);
        } else {
          console.error('[tcp] connect failed before pipe:', classified.code, classified.message, 'host:', header.host, 'port:', header.port);
        }
        if (sessionUserId) notifyPortalLog(ctx, RELAY_ID, 'error', sessionUserId, country, classified.code, PORTAL_URL, RELAY_SECRET);
        try { ws.close(1011, classified.code === 'tcp_timeout' ? 'TCP timeout' : 'Connect failed'); } catch {}
        await socket.close().catch(() => {});
        return;
      }

      if (sessionUserId) {
        notifyPortal(ctx, RELAY_ID, 'connect', sessionUserId, PORTAL_URL, RELAY_SECRET);
        notifyPortalLog(ctx, RELAY_ID, 'connect', sessionUserId, country, null, PORTAL_URL, RELAY_SECRET);
      }
      ...
      const initialData = buffer.length > header.dataOffset
        ...
        : null;
      debugLog('[pipe] initialData:', initialData ? initialData.length : 0, 'bytes');

      // ── Bidirectional pipe ─────────────────────────────────────────────────
      console.log('[session] connected host:', header.host, 'port:', header.port);
      const stats = await pipeStreams(ws, socket, reader, initialData);
      console.log('[session] closed host:', header.host, 'port:', header.port,
        'reason:', stats.closeReason,
        'wsToTcp:', stats.wsToTcpBytes,
        'tcpToWs:', stats.tcpToWsBytes);
      ...
      if (sessionUserId) {
        notifyPortal(ctx, RELAY_ID, 'disconnect', sessionUserId, PORTAL_URL, RELAY_SECRET);
        notifyPortalLog(ctx, RELAY_ID, 'disconnect', sessionUserId, country, null, PORTAL_URL, RELAY_SECRET);
      }
    }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/trojan' || url.pathname.startsWith('/trojan/') || url.pathname.startsWith('/trojan?')) {
      ...
      handleTrojanSession(server, ip, country, ctx).catch(() => {
