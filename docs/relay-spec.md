...
The relay script is a self-contained ES module with no external imports (except `cloudflare:sockets`). It is generated by `buildRelayScript()` in `packages/relay-worker` and uploaded by the portal Worker via the Cloudflare Workers REST API at deploy time.
